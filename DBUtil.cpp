...
{
	User user;
	char column[32][32];//用来保存数据库中查询到的行和列
	int res;
	string sql;
	if (isOpen)
	{
		...
		if (!res)
		{
			...
			if (result->row_count > 0)
			{
				...
				if (j > 0)
				{
					...
					user.m_nID = atoi(sql_row[0]);//C 库函数 int atoi(const char *str) 把参数 str 所指向的字符串转换为一个整数（类型为 int 型）
...
//显示热榜图书,根据书被借出的量
bool DBUtil:: SelectHotBooks(vector<Book>&books) {
	int res;
	string sql;
	if (isOpen)
	{
		mysql_query(&myCont, "SET NAMES GBK"); //设置编码格式,否则在cmd下无法显示中文
		sql += "select * from book order by (total - lefted)  desc limit 5";  
		res = mysql_query(&myCont, sql.c_str());//查询
		if (!res)
		{
			result = mysql_store_result(&myCont);//保存查询到的数据到result
...
bool DBUtil::AddBook(Book book)
{
	...
	char szTotal[16];
	char szLeftNum[16];
	sprintf(szTotal, "%d", book.GetTotalNum());//把格式化的book.GetTotalNum() 写入szTotal
...
//更新图书库存，(借书的时候)
bool DBUtil::UpdateBook(Book book) {
	string sql = "";
	char bookId[16];
	char szLeftNum[16];
	sprintf(bookId, "%d", book.GetBookID());
	sprintf(szLeftNum, "%d", book.GetLeftNum()-1);
	
	//cout <<"红楼梦剩余"<< szLeftNum << endl;
	...
	if (isOpen)
	{
		//获得时间
		mysql_query(&myCont, "SET NAMES GBK"); //设置编码格式,否则在cmd下无法显示中文
		sql = sql+ "update  book set  lefted =" + szLeftNum + " where id="+ bookId;
		bool is =mysql_query(&myCont, sql.c_str());
		if (is) {
			cout << "sql 执行不成功" << sql<<endl;
		}
	}
	else
	{
...
//更新图书的库存 （还书和借书的时候都可以用）
bool DBUtil::UpdateBookNum(Book book) {
	string sql = "";
	char bookId[16];
	char szLeftNum[16];
	sprintf(bookId, "%d", book.GetBookID());
...
bool DBUtil::AddBorrowRecord(BorrowRecord borrowRecord)
{
	...
	char szBookId[16];
	char szUserId[16];
	sprintf(szBookId, "%d", borrowRecord.m_nBookId);//把int转化成字符串
